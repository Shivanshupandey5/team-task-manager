...
const TaskListTable = ({tableData}) => {
    ...
    const getPriorityBadgeColor = (priority)=>{
        switch(priority){
            ...
            default: return  'bg-grey-100 text-gray-500 border border-gray-200';
        }
    };
  return (
                ...
                {tableData.map((task)=>(
                    <tr key={task._id} className='border-t border-gray-200'>
                        ...
                        <td className='py-4 px-4'>
                            <span className={`px-2 py-1 text-xs rounded inline-block ${getStatusBadgeColor(task.startus)}`}>{task.status}</span>
                        </td>
                        <td className='py-4 px-4'>
                            <span className={`px-2 py-1 text-xs rounded inline-block ${getPriorityBadgeColor}`}>{task.priority}</span>
                        </td>
